...
def find_user(email=None, user_name=None) -> user_models.CustomUser:
    """
    ...
    if email == None and user_name == None:
        ...
        try:
            user = user_models.CustomUser.objects.get(user_name=user_name)
            return user
...
def encode_jwt(user, hours,  platform="IOS", reaml="API"):
    """
...
def decode_jwt(jwt_token) -> dict:
    ...
    try:
        ...
        if payload.get('allow'):
            user = user_models.CustomUser.objects.get(email=payload.get('email'))
            return payload, user
        else:        
