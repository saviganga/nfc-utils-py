...
class JWTAuth(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # # check platform type to collect device token
        # if platform in ['IOS', 'ANDROID']:
        #     if request.data.get('device_token') == None:
        #         print('login platform error no device token')
        #         return Response(
        #             data={"status": "FAILED", "message": "Please pass in device token for Android and IOS authentication"},
        ...
        try:
            user = xauth_utils.find_user(
                serialized_data.validated_data.get("email", None), serialized_data.validated_data.get('user_name')
            )
        ...
        # if user.is_blocked:
        #     return Response(
        #         data={"status": "FAILED", "message": "User account is disabled please contact support"}, 
        #         status=status.HTTP_400_BAD_REQUEST
        #         )
